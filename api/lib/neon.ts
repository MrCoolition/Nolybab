...
export type NolybabSql = ReturnType<typeof neon>;

let cachedSql: NolybabSql | null = null;
let cachedConnectionString: string | null = null;
let schemaPromise: Promise<void> | null = null;

function databaseConnectionString(): string | null {
  const value = (process.env.NEON_DB_CONNECT ?? process.env.DATABASE_URL)?.trim();
  return value || null;
}

export function getNeonSql(): NolybabSql | null {
...
async function initializeSchema(sql: NolybabSql): Promise<void> {
...
export async function getReadyNeonSql(): Promise<NolybabSql | null> {
  const sql = getNeonSql();
  if (!sql) return null;

  if (!schemaPromise) {
    schemaPromise = initializeSchema(sql).catch((error: unknown) => {
      schemaPromise = null;
      throw error;
    });
  }
