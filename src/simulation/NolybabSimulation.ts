...
import { bestAutonomousPair, generateQuestion, initialDirectors, stepGamemasters } from './gamemasters';
...
export class NolybabSimulation {
  ...
  update(realSeconds: number): void {
    ...
    if (this.state.currentQuestion) {
      ...
      if (this.state.currentQuestion.secondsLeft <= 0) {
        ...
        this.weave(pair, lesson?.id, true);
      }
    } else {
      this.state.interludeSeconds = Math.max(0, this.state.interludeSeconds - seconds);
      if (this.state.interludeSeconds <= 0) {
        this.state.currentQuestion = generateQuestion(this.state, this.rng);
        this.syncRandomState();
        this.emit();
      }
  ...
  weave(voices: [VoiceId, VoiceId], lessonId?: string, autonomous = false): WeaveOutcome | null {
    ...
    const succeeded = this.rng.next() <= probability;
    let outcome: WeaveOutcome;

    if (succeeded && selectedLesson) {
      outcome = this.applyReframe(question.focus, voices, selectedLesson, probability, autonomous);
    } else if (succeeded) {
      outcome = this.applySynthesis(question.focus, voices, probability, autonomous);
    } else {
      outcome = this.applyProductiveMistake(question.focus, voices, probability, autonomous, selectedLesson);
    }

    relationship.exchanges += 1;
    relationship.strength = clamp(relationship.strength + (succeeded ? 0.065 : 0.035));
    relationship.tension = clamp(relationship.tension + (contrast - 0.24) * 0.08 - (succeeded ? 0.025 : -0.015));
    ...
    this.state.interludeSeconds = this.state.resolvedQuestions % 9 === 0 ? 8 : 5.2;
  ...
  private normalizeRestoredState(): void {
    ...
    if (!this.state.currentQuestion && this.state.interludeSeconds <= 0) {
      this.state.currentQuestion = generateQuestion(this.state, this.rng);
      this.syncRandomState();
    }
  }
  ...
  private stepCivicDrift(): void {
    ...
    const autonomousRatio = this.state.autonomousResponses / Math.max(1, this.state.resolvedQuestions);
    ...
    this.state.qualities.agency = lerp(this.state.qualities.agency, clamp(0.68 - autonomousRatio * 0.24), 0.0012);
  ...
  private applySynthesis(
    ...
    this.changeQualities(focus, voices, true, autonomous);
  ...
  private applyProductiveMistake(
    ...
    const a = VOICE_BY_ID[voices[0]];
    const b = VOICE_BY_ID[voices[1]];
    this.changeQualities(focus, voices, false, autonomous);
  ...
  private applyReframe(
    ...
    const hook = this.rng.pick(options);
  ...
  private changeQualities(focus: QualityKey, voices: [VoiceId, VoiceId], succeeded: boolean, autonomous: boolean): void {
    const a = VOICE_BY_ID[voices[0]];
    const b = VOICE_BY_ID[voices[1]];
    ...
    if (succeeded) {
      this.state.qualities[focus] = clamp(this.state.qualities[focus] + 0.047 + underheardBonus * 0.012, 0.06, 0.98);
      this.state.qualities.coherence = clamp(this.state.qualities.coherence + 0.018);
      this.state.qualities.plurality = clamp(this.state.qualities.plurality + underheardBonus * 0.022 - 0.003);
    ...
    if (autonomous) {
      ...
      this.state.qualities.agency = clamp(this.state.qualities.agency - Math.max(0.003, 0.013 - agencyProtection), 0.06, 0.98);
    }
  }
